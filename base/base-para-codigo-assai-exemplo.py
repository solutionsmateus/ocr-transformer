...
def detectar_precos_bboxes(bgr):
    ...
    for c in cnts:
        ...
        area = w*h
        ar = w/float(h)
        # filtros: tamanho mínimo, relação e posição (evita rodapé cabeçalho)
        if area<1500 or ar<1.2 or h<20: 
            continue
        if y>H*0.92 or y< H*0.05:  # ignora faixa superior “campanha” e rodapé
            continue
...
def extrair_itens_encarte(
    ...
    for bx in price_boxes:
        ...
        if not preco:
            # fallback suave: dilatar e tentar de novo
        ...
        if not preco:
            continue  # se não tem preço, ignora (provável ruído)

        # descrição acima do preço
        desc_roi = crop_descricao(bgr, bx)
        if desc_roi.size == 0:
            continue
        # OCR da descrição (sem números quebrando)
        desc_txt = ocr(desc_roi, conf="--oem 3 --psm 6 -l por")
        # limpeza básica: remove pedaços só de rodapé, “QUILO/CADA” etc. mantendo no fim
        linhas = [l.strip(" .,:;|") for l in desc_txt.splitlines() if l.strip()]
        # pega 1–3 linhas mais “textuais” (com letras e sem muito número)
        boas = []
        for l in linhas:
            if len(l) < 3: 
                continue
            if RX_PRECO.search(l):  # se aparecer preço por engano, pula
                continue
        ...
        if not boas:
            continue
        # heurística: últimas linhas costumam ser a melhor descrição
    ...
    # De-duplicar e filtrar quebras esquisitas
    df = pd.DataFrame(itens).drop_duplicates(subset=["Produto (Descrição)","Preço (Do Encarte)"])
    # Remove descrições de 1 palavra (quase sempre ruído)
